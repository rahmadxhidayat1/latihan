...
@endif
<a href="/student/create"><button style="width: 25%"
        class="btn text-start btn-outline-primary border-2 mb-2">Input</button></a>
<form class="row" action="{{ route('student.index') }}" method="get" enctype="multipart/form-data">
        ...
        <select name="filter" id="filter" class="form-select" style="width: 25%" name="major" id="">
            @foreach($majors as $major )
                <option value="{{ $major->id }}"
                    {{ request('filter') == $major->id ? 'selected' : '' }}>
                    {{ $major->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="pt-2">
        ...
        <input type="text" placeholder="Pencarian" style="width: 25%" name="search"
            value="{{ request('search') }}" class="form-control" id="exampleInputEmail1"
...
<table class="table bg-warning bg-opacity-75 table-bordered border-dark">
    ...
    <tbody class="text-start">
        @foreach($data as $item)
            <tr>
                <th scope="row" style="color: rgb(255, 248, 10)" class="">
                ...
                <td class="table-warning">
                    ...
                    <form
                        action="{{ route('student.destroy', ['student' =>$item->id]) }}"
                        method="POST">
                        @method('delete')
                        @csrf
